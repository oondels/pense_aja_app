...
/*****************************************************************************/
const files = "routes/data/ip.txt";
...
async function listaTable() {
  ...
  let resBuscaDados = await buscaDados.json();
  if (resBuscaDados.erro == false) {
    resBuscaDados.dados.forEach((element) => {
        ...
        button.addEventListener("click", async function (event) {
          ...
          if (resBuscaPenseAja.erro == false) {
            ...
            elimPerdaModal.innerHTML =
              ...
              `${
                resBuscaPenseAja.penseAja.movimento != ""
                  ? " | " + resBuscaPenseAja.penseAja.movimento.toUpperCase()
                  : ""
              }` +
              `${
                resBuscaPenseAja.penseAja.estoque != ""
                  ? " | " + resBuscaPenseAja.penseAja.estoque.toUpperCase()
                  : ""
              }` +
              `${
                resBuscaPenseAja.penseAja.espera != ""
                  ? " | " + resBuscaPenseAja.penseAja.espera.toUpperCase()
                  : ""
              }` +
              `${
                resBuscaPenseAja.penseAja.talento != ""
                  ? " | " + resBuscaPenseAja.penseAja.talento.toUpperCase()
                  : ""
              }` +
              `${
            ...
            if (
              parseInt(resBuscaPenseAja.penseAja.valor_a) != 0 ||
              parseInt(resBuscaPenseAja.penseAja.valor_b) != 0
            ) {
            ...
            if (
              ...
              document.getElementById(
                resBuscaPenseAja.penseAja.classificacao.toLowerCase()
              ).checked = true;
            } else {
          ...
          if (resBuscaPenseAja.penseAja.a3_mae != "") {
            document.getElementById("escolha").innerText =
              resBuscaPenseAja.penseAja.a3_mae;
          } else {
          ...
          aprovar.addEventListener("click", async function (event) {
            ...
            if (escolha == "") {
              required(
                "Classificação obrigatória, por favor selecione um tipo!"
              );
              return false;
            }
            const dataA = {
...
async function listaTableLista() {
  ...
  let resBuscaDadosLista = await buscaDadosLista.json();
  if (resBuscaDadosLista.dados == "") {
    showLoading("none");
    required("Filtro inválido");
  } else {
    resBuscaDadosLista.dados.forEach((element) => {
      let tr = tbody.insertRow();
  ...
  const buscaPA_Lista = () => {
    let line = document.querySelectorAll(".activeLista").length;
