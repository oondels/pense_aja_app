...
router.get("/engagement/:dassOffice", async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { dassOffice } = req.params;
    const { startDate, endDate } = req.query;
    ...
    const result = await DashboardService.getEngagementData(dassOffice, startDateParsed, endDateParsed);
